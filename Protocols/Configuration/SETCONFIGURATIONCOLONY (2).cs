...
{
    public class SETCONFIGURATIONCOLONY : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetConfigurationColony(
                int.Parse(commandInfo.CityKey), string.Concat(commandInfo.Colony), string.Concat(commandInfo.PostCode)
                )));
