...
{
    public class UPDATECONFIGURATIONSTATE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.UpdateConfigurationState(
                int.Parse(commandInfo.StateKey), string.Concat(commandInfo.State)
                )));
