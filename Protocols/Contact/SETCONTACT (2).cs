...
{
    public class SETCONTACT : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetContact(
                string.Concat(commandInfo.Nombre), string.Concat(commandInfo.Email), string.Concat(commandInfo.Asunto), string.Concat(commandInfo.Mensaje)
                )));
