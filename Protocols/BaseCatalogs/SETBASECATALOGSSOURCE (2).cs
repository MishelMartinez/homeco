...
{
    public class SETBASECATALOGSSOURCE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetBaseCatalogsSource(
                string.Concat(commandInfo.Source)
                )));
