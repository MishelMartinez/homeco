...
{
    public class UPDATEBASECATALOGSSOURCE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.UpdateBaseCatalogsSource(
                    int.Parse(commandInfo.SourceKey), string.Concat(commandInfo.Source)
                )));
