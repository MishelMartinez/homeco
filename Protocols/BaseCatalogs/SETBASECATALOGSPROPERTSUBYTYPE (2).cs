...
{
    public class SETBASECATALOGSPROPERTSUBYTYPE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetBaseCatalogsPropertySubType(
                    string.Concat(commandInfo.PropertySubType), string.Concat(commandInfo.Descripcion), int.Parse(commandInfo.PropertyTypeKey)
                )));
