...
{
    public class UPDATEBASECATALOGSSECTOR : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.UpdateBaseCatalogsSector(
                    int.Parse(commandInfo.SectorKey), string.Concat(commandInfo.Sector)
                )));
