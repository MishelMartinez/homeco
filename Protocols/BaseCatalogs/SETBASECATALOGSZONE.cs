...
{
    public class SETBASECATALOGSZONE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetBaseCatalogsZone(
                string.Concat(commandInfo.Zone), int.Parse(commandInfo.CityKey), int.Parse(commandInfo.ColonyKey)
                )));
