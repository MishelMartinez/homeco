...
{
    public class DELETEBASECATALOGSZONE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.DeleteBaseCatalogsZone(
                    int.Parse(commandInfo.ZoneKey)
                )));
