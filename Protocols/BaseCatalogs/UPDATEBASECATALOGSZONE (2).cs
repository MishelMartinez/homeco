...
{
    public class UPDATEBASECATALOGSZONE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.UpdateBaseCatalogsZone(
                    int.Parse(commandInfo.ZoneKey), string.Concat(commandInfo.Zone), int.Parse(commandInfo.CityKey)
                )));
