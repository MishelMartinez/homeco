...
{
    public class SETBASECATALOGSGROUNDTYPE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.SetBaseCatalogsGroundType(
                string.Concat(commandInfo.GroudType)
                )));
