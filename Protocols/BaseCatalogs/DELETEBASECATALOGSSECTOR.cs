...
{
    public class DELETEBASECATALOGSSECTOR : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.DeleteBaseCatalogsSector(
                    int.Parse(commandInfo.SectorKey)
                )));
