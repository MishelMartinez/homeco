...
{
    public class UPDATEBASECATALOGSGROUNDTYPE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.UpdateBaseCatalogsGroundType(
                    int.Parse(commandInfo.GroundKey), string.Concat(commandInfo.GroudType)
                )));
