...
{
    public class DELETEBASECATALOGSSOURCE : JsonSubCommand<PropertyMessage>
    {
        protected override void ExecuteJsonCommand(WebSocketSession session, PropertyMessage commandInfo)
        {
            ...
            session.Send(String.Format("{0}", App.DataAccess.DataAccessProperty.DeleteBaseCatalogsSource(
                    int.Parse(commandInfo.SourceKey)
                )));
